...
```{r}
### Cria banco de vars demograficas (sexo,idade,etc)

demog_21 <- map(data_list_21,~select(.x,V2009,V2010,VD3006,UF,V2007,VD2002,CHAVE))%>%
  bind_rows()
demog_22 <- map(data_list_22,~select(.x,V2009,V2010,VD3006,UF,V2007,VD2002,CHAVE))%>%
  bind_rows()
...
situacao_individio <- function(row,next_databases){ #lembrar do ultimo df ser vazio
  ...
  df_created <- data.frame("chave"=chave,"tempo"=NA, 
                           "censura"=NA,"causa"=NA,row.names = NULL)
...
create_data_by_trimester <- function(previous_data,actual_data,next_databases){
  ...
  cat("Porcentagem de indivíduos sem causa definida de saída do desemprego:",
...
empty_df <- data.frame(CHAVE=NA) # dataframe para encerrar o loop corretamente
...
sla <- final %>% filter(censura==1)

ggplot(sla,aes(x=V2007,y=tempo))+
geom_boxplot(fill="darkblue",width=0.5)+
